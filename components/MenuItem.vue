...
<template>
    <article class="flex flex-col justify-between gap-4 border-b border-forest/10 py-5 sm:flex-row sm:items-center"
        :class="{ 'opacity-45': item.ativo !== true }">
        <div class="relative min-w-0 flex-1">
            <div class="flex items-center gap-2">
                <h3 class="display text-xl font-bold">{{ item.sabor }}</h3><span
                    v-if="item.ativo === true && typeof item.estoque_atual === 'number' && item.estoque_atual > 0"
                    class="availability-tooltip cursor-help text-sm font-bold text-leaf" tabindex="0"
                    aria-label="Item disponível para pronta entrega">●<span class="availability-tooltip-content">Pronta
                        ...
                        unidades` : '' }}</span></span><span v-else-if="item.ativo === true"
                    class="availability-tooltip cursor-help text-sm font-bold text-gold" tabindex="0"
            ...
            <p v-if="item.ativo !== true" class="mt-1 text-xs font-bold uppercase tracking-widest text-tomato">
                Indisponível</p>
        </div>
        <div class="flex items-center justify-between gap-5 sm:justify-end"><strong class="shrink-0 text-lg">{{
            formatPrice(item.preco) }}</strong><button v-if="item.ativo === true" type="button"
